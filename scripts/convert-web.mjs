...
const FILES = [
  ...
  {file: '更新日志.md', dir: 'changelog', label: '更新日志', position: 7, split: false},
  {file: '开发指南.md', dir: 'dev', label: '开发指南', position: 8},
];
...
for (const t of TARGETS) {
  ...
  for (const f of FILES) {
    ...
    const groupDir = path.join(outDir, f.dir);
    cleanDir(groupDir);
    fs.mkdirSync(groupDir, {recursive: true});
    fs.writeFileSync(
      path.join(groupDir, '_category_.json'),
      JSON.stringify({label: f.label, position: f.position}, null, 2) + '\n',
      'utf8',
    );
